...
# Let's compare the difference in variance of returns between the raw and aggregated data
v1 = agg_1.VWAP.dropna().pct_change().var()
v2 = trades.Price.pct_change().var()
v3 = agg_2.Close.dropna().pct_change().var()
print(f'Agg Variance: {np.round(v1,5)} vs. Raw Price Variance: {np.round(v2,5)}')
print(f'Agg Variance: {np.round(v1,5)} vs. 2min Sample Price Variance: {np.round(v3,5)}')
